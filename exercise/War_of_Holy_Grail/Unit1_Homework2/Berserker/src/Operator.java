        ...
        for (Term object : left.getTerms()) {                                       // copy
            ...
            for (Factor key : object.getFactors().keySet()) {
                if (key.getPos() == 1) {
                    ...
                    Sin sin = new Sin(key.getCoefficient(), key.getPower());
                    factors.put(sin, z);
                } else if (key.getPos() == 2) {
                    BigInteger z = object.getFactors().get(key);
                    Cos cos = new Cos(key.getCoefficient(), key.getPower());
                    factors.put(cos, z);
                }
            }
    ...
    public ArrayList<Term> opSub() {
        ...
        for (Term object : left.getTerms()) {
            ...
            for (Factor key : object.getFactors().keySet()) {
                if (key.getPos() == 1) {
                    BigInteger z = object.getFactors().get(key);
                    Factor factor = new Sin(key.getCoefficient(), key.getPower());
                    factors.put(factor, z);
                } else if (key.getPos() == 2) {
                    BigInteger z = object.getFactors().get(key);
                    Factor factor = new Cos(key.getCoefficient(), key.getPower());
                    factors.put(factor, z);
                }
            }
            Term term = new Term(x, y, factors);
    ...
    public HashMap<Factor, BigInteger> mulTri(ArrayList<HashMap<Factor, BigInteger>> m) {
        ...
        if (a.size() == 0 && b.size() != 0) {
            for (Factor key : b.keySet()) {
                BigInteger kcoe = key.getCoefficient();
                BigInteger kpow = key.getPower();
                BigInteger pow = b.get(key);
                if (key.getPos() == 1) {
                    Sin sin = new Sin(kcoe, kpow);
                    c.put(sin, pow);
                } else if (key.getPos() == 2) {
                    Cos cos = new Cos(kcoe, kpow);
                    c.put(cos, pow);
                }
            }
        } else if (a.size() != 0) {

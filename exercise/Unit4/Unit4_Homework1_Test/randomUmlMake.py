...
def makeOperation(builder, parentid, inputcount, isreturn, visibility, opname, parentlist):
    # inmode = ['in', 'out', 'inout']
    opid = builder.createOperation(parentid, opname, visibility)
    for i in range(inputcount):
        #mode = inmode[random.randint(0, 2)]
        mode = 'in'
        builder.createParameter(opid, PARA + str(i), makeParameterType(isreturn, parentlist), mode)
    if (isreturn):
        builder.createParameter(opid, 'return', makeParameterType(isreturn, parentlist), 'return')

def makeParameterType(isreturn, parentlist):
    nameType = ['int', 'byte', 'char', 'float', 'String', 'double', 'long', 'boolean', 'short', 'void']
    if (random.choice([False, True]) or len(parentlist) == 0):
        if (isreturn):
            return nameType[random.randint(0, 9)]
        else:
            return nameType[random.randint(0, 8)]
    else:
        if (random.choice([False, True])) :
            return '{\"$ref\":\"' + parentlist[len(parentlist) - 1] + '\"}' #增大相同类型的概率
        else :
            return '{\"$ref\":\"' + random.choice(parentlist) + '\"}'
    ...
    for i in range(total):
        ...
        while (opcount < operationtotal):
            ...
            while (str(inputcount) + str(isreturn) + str(visibility) in oppara):
                ...
                isreturn = random.choice([True, False])
                visibility = makeVisibility()
            makeOperation(builder, parentid, inputcount, isreturn, visibility, makeOperationName(i, opnum, isClass), parentlist)
        attricount = 0
...
def randomAssociation(builder, l):
    for i in range(associationtotal):
        end1 = random.choice(l)
        end2 = random.choice(l)
...
class RandomUmlMake:
    def __init__(self, filecount):
        file = './data.json'
        file_struct = './data_struct.txt'
        self._file = open(file, 'w')
        self._struct = open(file_struct, 'w')

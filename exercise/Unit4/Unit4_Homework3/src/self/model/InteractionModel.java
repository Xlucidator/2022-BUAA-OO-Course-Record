...
public class InteractionModel {
    ...
    private final HashSet<String> lifelineIdDestroyed = new HashSet<>();
    /* ======= Exception ======= */
    private boolean umlR007 = false;
    ...
    public void checkLifelineRepresent(HashSet<String> repIdSet) throws UmlRule006Exception {
        for (UmlLifeline lifeline: id2Lifeline.values()) {
            if (!repIdSet.contains(lifeline.getRepresent())) {
                throw new UmlRule006Exception();
            }
        }
    }

    public void checkReceiveAfterDestroyed() throws UmlRule007Exception {
        if (umlR007) {
    ...
    public void addMessage(UmlMessage umlMessage) {
        ...
        if (lifelineIdDestroyed.contains(tarId)) {
            umlR007 = true;
        }

        if (umlMessage.getMessageSort() == MessageSort.DELETE_MESSAGE) {
            lifelineIdDestroyed.add(tarId);
        } else if (umlMessage.getMessageSort() == MessageSort.CREATE_MESSAGE) {
